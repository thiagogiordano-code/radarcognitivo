...
app.post('/api/analyze', async (req, res) => {
  ...
  if (!apiKey) {
    console.error('[analyze] GEMINI_API_KEY ausente');
  ...
  const promptText = `Atue como o Prof. Thiago Giordano Siqueira, especialista em Andragogia e Design Instrucional.
...
4. Encerre com um tom motivador e profissional, dirigindo-se ao aluno como "você".`;
  ...
  console.log('[analyze] iniciando chamada Gemini para curso:', profile.course);
  ...
  try {
    ...
    console.log('[analyze] resposta recebida, tamanho:', text?.length ?? 0);
    res.json({ text: text || 'Não foi possível gerar a análise detalhada no momento.' });
  } catch (error) {
